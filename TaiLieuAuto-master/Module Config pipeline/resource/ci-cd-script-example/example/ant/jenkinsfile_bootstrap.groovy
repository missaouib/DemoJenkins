...
def bootstrap_roll_back(){
    ...
    try {
        ...
        //Credential of user
       withCredentials([usernamePassword(credentialsId: "$env.GITLAB_CREDENTIALID", usernameVariable: 'username',
...
def bootstrapDeployToProduction() {
    ...
    try {
        ...
        withCredentials([usernamePassword(credentialsId: "$env.GITLAB_CREDENTIALID", usernameVariable: 'user',
              passwordVariable: 'password')]){
            def issueContentJson = """
                                    {
                                        "title": "${env.DEPLOY_RESULT_TITLE}",
...
def isOpenMergeRequest(gitlabMergeRequestIid) {
    withCredentials([
        usernamePassword(credentialsId: "$env.GITLAB_CREDENTIALID", usernameVariable: 'username', passwordVariable: 'password'),

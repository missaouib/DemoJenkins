...
def bootstrap_roll_back(){
    ...
    try {
        ...
        //Credential of user
       withCredentials([usernamePassword(credentialsId: 'a5eedd9f-332d-4575-9756-c358bbd808eb', usernameVariable: 'username',
                ...
                if(env.mailTo == null){
                    ...
                    mail([
                        ...
                        body: " <summary>Push Commit <strong>${env.gitlabSourceBranch}:${lastCommitShortName}</strong>: " +
                            ...
                            "<br/> UnitTest Result: ${env.UNIT_TEST_RESULT_STR}" +
                            "<br/> CODE COVERAGE Result: ${env.CODE_COVERAGE_RESULT_STR}",
                        mimeType: 'text/html',
                        cc: "${env.mailCC}",
...
@SuppressWarnings("GroovyAssignabilityCheck")
def checkIfBuildResultSuccess(buildURL) {
    withCredentials([usernamePassword(credentialsId: 'jenkins_api_token_new', usernameVariable: 'usernamejenkins', passwordVariable: 'token')]) {
        String basicAuthString = "Basic " + "${usernamejenkins}:${token}".bytes.encodeBase64().toString()
        def buildInfoResp = httpRequest([
            acceptType   : 'APPLICATION_JSON',
            httpMode     : 'GET',
            contentType  : 'APPLICATION_JSON',
        ...
        return jenkinsfile_utils.jsonParse(buildInfoResp.content)["result"] == "SUCCESS"
    }
}
...
/*
  Các fuctions đươc sử dụng khi build lib dùng chung cho các service khác.
*/
def removeLibNexus(){
    withCredentials([usernamePassword(credentialsId: '5a87e1f9-d160-4e56-b06c-4158622898be', usernameVariable: 'user', passwordVariable: 'password')]){
            String basicAuthString = "Basic " + "${user}:${password}".bytes.encodeBase64().toString()
            def runTask = httpRequest([
                        httpMode                : 'POST',
                        acceptType              : 'APPLICATION_JSON',
                        contentType             :  'APPLICATION_JSON',
                        consoleLogResponseBody  : true,
                        authentication: '5a87e1f9-d160-4e56-b06c-4158622898be',
                        url                     : "${env.NEXUS_REPO_LIB_URL}/tasks/${env.IDTask}/run"

            ])
            println("Status: "+ runTask.status)
        }
}
def getResultNexus(){
        sleep 10
        withCredentials([usernamePassword(credentialsId: '5a87e1f9-d160-4e56-b06c-4158622898be', usernameVariable: 'user', passwordVariable: 'password')]){
            def getAssets = httpRequest([
                        httpMode                : 'GET',
                        acceptType              : 'APPLICATION_JSON',
                        contentType             :  'APPLICATION_JSON',
                        authentication          : '5a87e1f9-d160-4e56-b06c-4158622898be',
                        url                     : "${env.NEXUS_REPO_LIB_URL}/search?repository=${env.REPO_NAME}&maven.groupId=${env.MAVEN_GROUP_NAME}&name=${env.MAVEN_ARTIFACT_NAME}"

            ])
            println("Status getAsset : "+ getAssets.status)
            println("Content : " + jenkinsfile_utils.jsonParse(getAssets.content))
            def getStatusTask = httpRequest([
                        httpMode                : 'GET',
                        acceptType              : 'APPLICATION_JSON',
                        contentType             :  'APPLICATION_JSON',
                        authentication          : '5a87e1f9-d160-4e56-b06c-4158622898be',
                        url                     : "${env.NEXUS_REPO_LIB_URL}/tasks/${env.IDTask}"

            ])
            println("Status get Task : "+ getStatusTask.status)
            println("Content : " + jenkinsfile_utils.jsonParse(getStatusTask.content))
            Map convertedJSONMap = jenkinsfile_utils.jsonParse(getStatusTask.content)
            while (convertedJSONMap."currentState" == "RUNNING"){
                sleep 10
                println("Content get task after sleep: " + jenkinsfile_utils.jsonParse(getStatusTask.content))
            }

        }
}
def checkjobBuildLibRunning(){
    while(checkIfBuildIsRunning(env.buildUrlLib)){
        sleep 10
    }
    echo "$env.buildUrlLib building complete"
    if(!checkIfBuildIsRunning(env.buildUrlLib) && checkIfBuildResultSuccess(env.buildUrlLib)){
         println "$env.buildUrlLib Build success"
    }else{
         error "build faild"
    }

}
def removeRepoNexusAndcleanEnviroment(){
    stage("Remove Lib in Nexus"){
        removeLibNexus()
        getResultNexus()
    }
    def cleanEnvs = [:]
            cleanEnvs['clean environment in slave 43'] = {
                node("slave_43"){
                    checkout changelog: true, poll: true, scm: [
                        ...
                        userRemoteConfigs                : [[credentialsId: "63265de3-8396-40f9-803e-5cd0b694e519",
                                                            url          : "http://10.60.156.11/hienptt22/sharelibrary" +".git"]]
                        ]
                    sleep 5
                    sh """
                    pwd
                    ls -la
                    sh $env.fileNameRemoveLibs
                    """
                    echo "finish remove folder"
                    sh "rm -rf $env.fileNameRemoveLibs"
                }
            }
            cleanEnvs['clean environment in slave 68'] = {
                node("node_cicd"){
                    checkout changelog: true, poll: true, scm: [
                        $class                           :  'GitSCM',
                        branches                         : [[name: "master"]],
            ...
            cleanEnvs['clean environment in slave 116'] = {
                node("slave_116"){
                    checkout changelog: true, poll: true, scm: [
                        $class                           :  'GitSCM',
                        branches                         : [[name: "master"]],

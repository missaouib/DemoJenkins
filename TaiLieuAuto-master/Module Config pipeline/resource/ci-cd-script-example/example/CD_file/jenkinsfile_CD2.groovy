def deployToProduction() {
    ...
    def deployInput = ""
    def deployer = ""
    env.project_version = env.config_git_branch
    println "Version deploy: ${env.project_version}"
    stage("2. Wait for maintainer accept or reject to deploy to production") {
        try {
            ...
            timeout(time: 24, unit: 'HOURS') {
                deployInput = input(
                    ...
                    choice(choices: ['Deploy', 'Abort'], description: "Deploy to production or abort deploy process with version $env.project_maintainer_list ?", name: 'DEPLOY_CHOICES')
                ])
            }
        ...
        if(deployer.contains(deployInput.submitter) && deployInput.DEPLOY_CHOICES == "Deploy"){
            stage('3. Deploy to Dev'){
                echo "Tag: ${env.GIT_TAG_DEPLOY}"
                jenkinsfile_CI.release2k8s("$env.project_version","idm","auth-service", "PUSH")
            }

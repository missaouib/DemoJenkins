...
def bootstrap_roll_back(){
    ...
    try {
        ...
        //Credential of user
       withCredentials([usernamePassword(credentialsId: '63265de3-8396-40f9-803e-5cd0b694e519', usernameVariable: 'username',
...
//send mail when sonar faild ==> chưa bổ sung phần build faild
            ...
            // mail bcc: '', body: " $buildResultStr <br/> ${env.SONAR_QUBE_SCAN_RESULT_STR}", mimeType: 'text/html', cc: '', from: 'vtsjenkinsadmin@viettel.com.vn', replyTo: '', subject: "$JOB_NAME - Build # $BUILD_NUMBER - $currentBuild.result!", to: 'hienptt22@viettel.com.vn'
            // mail bcc: '', body: " <summary>Push Commit <strong>${env.gitlabSourceBranch}:${lastCommitShortName}</strong>: " + "<br/> ${buildResultStr}</summary> <br/> <h4><i><a href='${env.BUILD_URL}display/redirect'>" + "Build Details...</a></i></h4><br/>Sonar Result: ${env.SONAR_QUBE_SCAN_RESULT_STR}", mimeType: 'text/html', cc: 'truongdx8@viettel.com.vn, linhnv10@viettel.com.vn, quanpv2@viettel.com.vn, hoahv5@viettel.com.vn', from: 'vtsjenkinsadmin@viettel.com.vn', replyTo: '', subject: "$JOB_NAME - Build # $BUILD_NUMBER - $currentBuild.result!", to: 'huongnv75@viettel.com.vn'
        }
    }
}
...
def bootstrapMergeRequestBuild() {
    try {
        ...
        if (currentBuild.result == "FAILURE") {
            // TODO Rollback when build failed
        }
    }
}
